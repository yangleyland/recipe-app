    ...
    <main className="min-h-screen text-[var(--ink)]">
      ...
      <article className="px-5 py-8 sm:px-8 lg:px-12">
        <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <aside className="rounded-[2rem] border border-pink-100 bg-white/72 p-7 shadow-xl shadow-rose-900/5 lg:sticky lg:top-36 lg:self-start">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <DeleteRecipeForm id={recipe.id} title={recipe.title} />
            </div>
          </aside>

          <div className="grid gap-6">
            <section className="rounded-[2rem] border border-pink-100 bg-white/72 p-6 shadow-xl shadow-rose-900/5 sm:p-8">
              <h2 className="font-serif text-4xl font-black">Ingredients</h2>
              <ul className="mt-5 grid gap-3">
                {recipe.ingredients.map((ingredient) => (
                  <li key={ingredient} className="rounded-2xl bg-white/70 px-4 py-3 font-bold text-pink-950">
                    {ingredient}
                  </li>
                ))}
              </ul>
            ...
            <section className="rounded-[2rem] border border-pink-100 bg-white/72 p-6 shadow-xl shadow-rose-900/5 sm:p-8">
              ...
              <ol className="mt-5 grid gap-3">
                {recipe.instructions.map((instruction, index) => (
                  <li key={instruction} className="flex gap-3 rounded-2xl bg-white/70 px-4 py-3 leading-7 text-pink-950">
